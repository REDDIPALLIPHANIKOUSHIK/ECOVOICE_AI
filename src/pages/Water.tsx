...
import { Droplets, Search, Recycle as RecycleIcon, Activity, Lightbulb, AlertTriangle, CheckCircle, Info } from "lucide-react";
...
const severityColor: Record<string, string> = {
  low: "text-primary",
  medium: "text-eco-yellow",
  high: "text-eco-orange",
  critical: "text-destructive",
};

const classColor: Record<string, string> = {
  reusable: "text-primary",
  partially_reusable: "text-eco-yellow",
          ...
          <div className="space-y-4">
              ...
              <div className="eco-card p-6 animate-scale-in space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-muted rounded-xl p-3 text-center">
                    <p className="text-xl font-bold text-eco-blue">{wasteResult.dailyLiters}L</p>
                    <p className="text-xs text-muted-foreground">Per Day</p>
                  </div>
                  <div className="bg-muted rounded-xl p-3 text-center">
                    <p className="text-xl font-bold text-eco-blue">{wasteResult.monthlyLiters.toLocaleString()}L</p>
                    <p className="text-xs text-muted-foreground">Per Month</p>
                  </div>
                  <div className="bg-muted rounded-xl p-3 text-center">
                    <p className="text-xl font-bold text-eco-blue">{wasteResult.yearlyLiters.toLocaleString()}L</p>
            ...
            {wasteInput && !wasteResult && (
              <div className="eco-card p-6 text-center text-muted-foreground">
                <p className="text-sm">
                  No exact match found. Try: "leaking tap", "running toilet", "overflowing tank", "dripping shower", "garden hose", or "leaking pipe"
                </p>
              </div>
            )}
          </div>
        ...
        {activeTab === "reuse" && (
          <div className="space-y-4">
            ...
            {reuseInput && !reuseResult && (
              <div className="eco-card p-6 text-center text-muted-foreground">
                <p className="text-sm">
                  Try: "washing machine", "rice water", "bath water", "cooking water", "AC water", or "RO reject"
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="eco-card p-6">
              <h2 className="font-display font-semibold mb-1 flex items-center gap-2">
                <Activity className="w-5 h-5 text-eco-blue" />
                Your Daily Water Footprint
              </h2>
              <p className="text-sm text-muted-foreground mb-4">
                See how much water common activities use — and how to cut down
              </p>
                ...
                <div key={i} className="eco-card p-4 animate-fade-up" style={{ animationDelay: `${i * 0.05}s` }}>
                  ...
                  <div className="h-1.5 rounded-full bg-border overflow-hidden mb-2">
                    <div
                      ...
                      style={{ width: `${(item.optimizedLiters / item.litersUsed) * 100}%` }}
                    />
                  </div>
                  <ul className="flex flex-wrap gap-1.5">
                    {item.savingTips.map((tip, j) => (
                      <li key={j} className="text-xs bg-secondary px-2 py-0.5 rounded-full text-secondary-foreground">
                        {tip}
                      </li>
                    ))}
                  </ul>
